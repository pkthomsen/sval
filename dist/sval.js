    ...
    ***************************************************************************** */
    ...
    function getGetterOrSetter(method, obj, key, ctrl) {
        ...
        if (method === "set" && ctrl.blackList.includes(obj)) {
            var idx = ctrl.blackList.findIndex(function (p) { return p === obj; });
    ...
    try {
        ...
        try {
            ...
            try {
                globalObj.atob = function (str) { return atob(str); };
            }
            catch (err) { }
            try {
                globalObj.btoa = function (str) { return btoa(str); };
            }
            catch (err) { }
            try {
    ...
    function createPlainJsGlobalObj() {
        ...
        var coreJs = {
            Object: Object, Function: Function, Array: Array, Number: Number, String: String, Boolean: Boolean,
            Infinity: Infinity, NaN: NaN, undefined: undefined,
            Date: Date, Error: Error,
            parseFloat: parseFloat, parseInt: parseInt, Math: Math, JSON: JSON, isNaN: isNaN, isFinite: isFinite,
        };
        Object.assign(globalObj, coreJs);
        try {
            Object.assign(globalObj, { decodeURI: decodeURI, decodeURIComponent: decodeURIComponent, encodeURI: encodeURI, encodeURIComponent: encodeURIComponent,
                atob: function (str) { return atob(str); }, btoa: function (str) { return btoa(str); } });
        }
        catch (err) { }
        return globalObj;
    }
    var emptyGlobalObj = createPlainJsGlobalObj();
    var emptyWin = create({});
    Object.assign(emptyWin, emptyGlobalObj);
    function createSandBox(type) {
        var _a, _b;
        if (type === "empty") {
            return assign(create((_a = {}, _a[WINDOW] = emptyGlobalObj, _a)), emptyWin);
        }
        else {
            return assign(create((_b = {}, _b[WINDOW] = globalObj, _b)), win);
        }
    }
    ...
    var Prop = (function () {
        function Prop(object, property, ctrl) {
        ...
        Prop.prototype.set = function (value) {
            ...
            if (this.ctrl.blackList.includes(this.object)) {
                var idx = this.ctrl.blackList.findIndex(function (p) { return p === _this.object; });
    ...
    var Scope = (function () {
        ...
        Scope.prototype.entry = function () {
        ...
        Scope.prototype.setTimeout = function (timeout) {
    ...
    function MemberExpression(node, scope, options) {
        ...
        if (getVar) {
            var setter = getSetter(object, key, scope.ctrl);
            ...
            var getter = getGetter(object, key, scope.ctrl);
    ...
    function CallExpression(node, scope) {
        ...
        if (scope.ctrl.interceptFunction.includes(func)) {
            var idx = scope.ctrl.interceptFunction.findIndex(function (p) { return p === func; });
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 7:
                    ...
                    if (getVar) {
                        setter = getSetter(object, key, scope.ctrl);
                        if (node.object.type === 'Super' && setter) {
                            thisObject = scope.find('this').get();
                            privateKey = createSymbol(key);
                            define(thisObject, privateKey, { set: setter });
                            return [2, new Prop(thisObject, privateKey, scope.ctrl)];
                        }
                        else {
                            return [2, new Prop(object, key, scope.ctrl)];
                        }
                    }
                    else {
                        getter = getGetter(object, key, scope.ctrl);
                        if (node.object.type === 'Super' && getter) {
                            thisObject = scope.find('this').get();
                            if (node.optional && thisObject == null) {
                                return [2, undefined];
    ...
    function CallExpression$1(node, scope) {
        var func, object, key, priv, obj, thisObject, name_1, idx, args, i, arg, _a, _b, _c, _d, superCall;
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                ...
                case 7:
                    if (scope.ctrl.interceptFunction.includes(func)) {
                        idx = scope.ctrl.interceptFunction.findIndex(function (p) { return p === func; });
                        func = scope.ctrl.interceptFunctionReplace[idx];
                        if (!func) {
                            throw new Error("calling function '" + scope.ctrl.interceptFunctionName[idx] + "' not allowed");
                        }
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 4:
                    ...
                    _c.label = 5;
                case 5:
                    if (!(statement.type === 'ForStatement' && ((_b = statement.init) === null || _b === void 0 ? void 0 : _b.type) === 'VariableDeclaration')) return [3, 7];
    ...
    var blackList = blackListDefs.map(function (p) { return p[0]; });
    var blackListNames = blackListDefs.map(function (p) { return p[1]; });
    var safeObjectAssign = function (target) {
        var sources = [];
        for (var _i = 1; _i < arguments.length; _i++) {
            sources[_i - 1] = arguments[_i];
        }
        if (blackList.includes(target)) {
            var idx = blackList.findIndex(function (p) { return p === target; });
            throw new Error("object '" + blackListNames[idx] + "' is readonly");
        }
        return Object.assign.apply(Object, __spread([target], sources));
    };
    var interceptFunctionDefs = [
    ...
    var interceptFunction = interceptFunctionDefs.map(function (p) { return p[0]; });
    var interceptFunctionName = interceptFunctionDefs.map(function (p) { return p[1]; });
    var interceptFunctionReplace = interceptFunctionDefs.map(function (p) { return p[2]; });
    var Sval = (function () {
